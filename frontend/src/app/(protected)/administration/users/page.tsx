...
const MODULES = ['Case', 'Notes', 'Snapshot', 'Reports', 'Tasks', 'Accounts'] as const;
...
export default function AdministrationUsersPage() {
  ...
  const [moduleRights, setModuleRights] = useState<Record<string, string>>(
  ...
  const validate = () => {
    ...
    if (!formData.rights) e.rights = 'Required';
    setErrors(e);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      console.log('Send invitation:', formData, moduleRights);
    }
  };
  ...
  const staticUsers = [
    { lastName: 'Thompson', firstName: 'David',   email: 'david.thompson@example.com',  country: 'United States',  cell: '+1 (555) 234-5678', role: 'Org Admin', rights: 'Full'  },
    { lastName: 'Wilson',   firstName: 'Sarah',   email: 'sarah.wilson@example.co.uk',  country: 'United Kingdom', cell: '+44 7911 123456',   role: 'Staff',     rights: 'View'  },
    { lastName: 'Chen',     firstName: 'Michael', email: 'michael.chen@example.com',    country: 'Australia',      cell: '+61 412 345 678',   role: 'Finance',   rights: 'Full'  },
    { lastName: 'Patel',    firstName: 'Priya',   email: 'priya.patel@example.ca',      country: 'Canada',         cell: '+1 (604) 555-0177', role: 'Role 1',    rights: 'View'  },
  ];
  ...
  return (
    <>
      <Container>
        <Toolbar>
          <ToolbarHeading>
            <ToolbarPageTitle text="Administration Users" />
          </ToolbarHeading>
        </Toolbar>
      </Container>
              ...
              <div className="flex flex-col gap-6">
                <SectionBox title="Account">
                  <div className="flex flex-col gap-4">
                    ...
                    <Field label="Rights" required error={errors.rights}>
                      <Select value={formData.rights} onValueChange={(v) => handleChange('rights', v)}>
                        <SelectTrigger className={cn('h-9 text-sm w-full', errors.rights ? 'border-destructive' : '')}>
                ...
                <div className="flex items-center justify-end gap-3">
                  <Button
                    ...
                    type="button"
                    onClick={() => {
                      setFormData({ lastName: '', firstName: '', email: '', username: '', country: '', cell: '', role: '', rights: '' });
                      setErrors({});
                    }}
                  >
                    Cancel
      ...
      {/* ── Users Grid ── */}
      <Container>
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden mt-[20px]">
          <div className="bg-accent/60 px-4 py-2 border-b border-border">
            <span className="text-xs font-semibold text-mono uppercase tracking-wide">Users List</span>
          </div>
          <div className="overflow-x-auto">
            <div className="grid grid-cols-6 border-b border-border text-xs font-semibold text-mono bg-muted/30 min-w-[700px]">
              ...
              <span className="px-3 py-2 border-r border-border">Role</span>
              <span className="px-3 py-2">Rights</span>
            </div>
            {staticUsers.map((row, i) => (
              <div key={i} className="grid grid-cols-6 border-b border-border last:border-b-0 min-w-[700px] hover:bg-accent/30 transition-colors">
                ...
                <span className="px-3 py-2 border-r border-border text-sm">{row.role}</span>
                <span className="px-3 py-2 text-sm">{row.rights}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Assign Roles Grid ── */}
      <Container>
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden mt-[20px] mb-6">
          <div className="bg-accent/60 px-4 py-2 border-b border-border">
            <span className="text-xs font-semibold text-mono uppercase tracking-wide">Assign Roles</span>
          </div>
          <div className="overflow-x-auto">
            <div className="grid grid-cols-2 border-b border-border text-xs font-semibold text-mono bg-muted/30">
              <span className="px-3 py-2 border-r border-border">Module Name</span>
              <span className="px-3 py-2">Rights</span>
            </div>
            {MODULES.map((mod) => (
              <div key={mod} className="grid grid-cols-2 border-b border-border last:border-b-0 hover:bg-accent/30 transition-colors">
                <span className="px-3 py-2 border-r border-border text-sm flex items-center">{mod}</span>
                <div className="px-3 py-2">
                  <Select
                    value={moduleRights[mod]}
                    onValueChange={(v) => setModuleRights((prev) => ({ ...prev, [mod]: v }))}
                  >
