...
const rows = [
  { label: 'Overdue', count: 6, color: 'bg-destructive' },
  { label: 'Due Today', count: 11, color: 'bg-warning' },
  { label: 'Upcoming', count: 29, color: 'bg-primary' },
  { label: 'Completed', count: 43, color: 'bg-success' },
];

export function PendingTasks() {
  const total = rows.slice(0, 3).reduce((s, r) => s + r.count, 0);
