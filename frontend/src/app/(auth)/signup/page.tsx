...
export default function Page() {
  ...
  async function onSubmit(_values: SignupSchemaType) {
    setIsProcessing(true);
    setError(null);

    // TODO: Uncomment backend call when ready
    // try {
    //   await signup({ name: _values.name, email: _values.email, password: _values.password }).unwrap();
    //   setSuccess(true);
    // } catch (err: any) {
    ...
    // Static mock signup — redirect to email verification
    setTimeout(() => {
      setIsProcessing(false);
      router.push(`/verify-email?email=${encodeURIComponent(_values.email)}&name=${encodeURIComponent(_values.name)}`);
    }, 800);
  }
  ...
  if (success) {
    return (
      <Alert onClose={() => setSuccess(false)}>
        ...
        <AlertTitle>
          You have successfully signed up! Please check your email to verify your account and
          then{' '}
